...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cargarSalasModerador = async () => {
            ...
            misSalas.forEach(sala => {
                ...
                div.innerHTML = `
                    <div class="flex justify-between items-start mb-3">
                        <div>
                            <p class="font-bold text-gray-800 dark:text-white">${sala.nombre}</p>
                            <p class="text-[10px] uppercase font-bold text-orange-500 dark:text-orange-400">Límite: ${textoLimite}</p>
                        </div>
        ...
        } catch (error) { listaMisSalas.innerHTML = '<p class="text-red-500">Error.</p>'; }
    };
                ...
                pendientes.forEach(p => {
                    ...
                    card.innerHTML = `
                        <div class="flex justify-between items-start mb-2">
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-200"><i class="fas fa-user mr-1"></i> ${p.autorNombre}</span>
                            <span class="text-xs text-gray-400 dark:text-gray-500">${new Date(p.fechaCreacion).toLocaleString()}</span>
                        </div>
                        ...
                        <div class="flex gap-2 flex-wrap mb-3">
                            ...
                            <button data-autor="${p.autorId}" class="btn-avisar bg-gray-800 dark:bg-gray-600 text-white px-3 py-1 rounded text-sm font-bold hover:bg-black transition-colors ml-auto"><i class="fas fa-envelope"></i> Avisar</button>
                        </div>
                        <div class="flex gap-2 flex-wrap pt-3 border-t dark:border-gray-700">
                            ...
                            <button data-autor="${p.autorId}" class="btn-baja text-purple-700 dark:text-purple-400 hover:underline text-xs font-bold"><i class="fas fa-gavel"></i> Solicitar Baja Global</button>
                        </div>
                    `;
    ...
    const cargarParticipantes = async (salaId) => {
        ...
        try {
            ...
            usuarios.forEach(u => {
                const card = document.createElement('div');
                card.className = "bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col justify-between transition-colors";
                card.innerHTML = `
                    <div class="mb-3">
                        <p class="font-bold text-gray-800 dark:text-white"><i class="fas fa-user text-blue-500 mr-2"></i>${u.nombre}</p>
                    ...
                    <div class="flex flex-wrap gap-2 mt-2 pt-2 border-t dark:border-gray-700">
                        <button data-autor="${u.id}" class="btn-avisar flex-1 bg-gray-800 dark:bg-gray-600 text-white hover:bg-black px-2 py-1 rounded text-xs font-bold"><i class="fas fa-envelope"></i> Avisar</button>
    ...
    const asignarEventosGenerales = (salaId, nombreSala) => {
        document.querySelectorAll('.btn-procesar').forEach(b => b.addEventListener('click', async (e) => {
            await fetch(`${API_BASE_URL}/moderacion/publicacion/${e.currentTarget.dataset.id}/estado?nuevoEstado=${e.currentTarget.dataset.estado}`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }});
            cargarPendientes(salaId, nombreSala);
        }));

        document.querySelectorAll('.btn-avisar').forEach(b => b.addEventListener('click', async (e) => {
            const msg = prompt("Escribe el aviso privado:");
            if(msg) await fetch(`${API_BASE_URL}/moderacion/avisar-usuario?destinoId=${e.currentTarget.dataset.autor}`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'text/plain' }, body: msg});
        }));
        ...
        document.querySelectorAll('.btn-bloquear').forEach(b => b.addEventListener('click', async (e) => {
            ...
            if(confirm(`Bloquear por ${dias} dias?`)) await fetch(`${API_BASE_URL}/moderacion/avanzada/bloquear?usuarioId=${autor}&salaId=${sala}&dias=${dias}`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }});
        }));
    };
